...
mxProp3 <- mxProp %>%
  ...
  filter(location_sampled == "boat") %>%
  group_by(sample_type, sample_description, station, boat_name, transit, direction, Source) %>%
  summarise(
    meanProp = mean(Proportion)
  )
head(mxProp3)
...
ggplot(mxProp3, aes(x=station, y = meanProp))+
  geom_col(mapping = aes(fill = Source), color = "black", linewidth = 0.1, position = "fill", show.legend = TRUE)+
  facet_nested(cols = vars(direction, boat_name), rows = vars(sample_type, sample_description), space = "free", scales = "free")+
  ylab("Proportion of Community") +
  scale_fill_manual(values = colors) +
...
mxProp4 <- mxProp %>% 
  ...
  mutate(station = as.numeric(station),
         direction = ifelse(transit == "t1" | transit == "t3", "Departing", "Returning")) %>%
  #filter(location_sampled == "boat") %>%
  group_by(sample_type, sample_description, station, boat_name, transit, true_geographic_order, direction, Source) %>%
  summarise(
    meanProp = mean(Proportion)
  ) %>%
