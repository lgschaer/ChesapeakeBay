...
sdata <- as.csv("/home/lgschaer/old/Chesapeake/March_2023_Analysis/phyloseq_out/full_cleaned_metadata_edited04112023.csv", row.names = 1, header = TRUE, sep = ",", check.names = TRUE, stringsAsFactors = TRUE) %>%
  mutate(sample_detail_description2 = case_when(
    sample_detail_description == "blank" ~ "Blank",
    sample_detail_description == "boat_surface_transom" ~ "Boat Surface Transom",
    sample_detail_description == "boat_bilge_biofilm" ~ "Boat Bilge Biofilm",
    sample_detail_description == "boat_surface_hull" ~ "Boat Surface Hull",
    sample_detail_description == "boat_bilge_water" ~ "Boat Bilge Water",
    sample_detail_description == "openwater_filter_PES_0.2_um" ~ "Open Water 0.2 um PES Filter",
    sample_detail_description == "boat_bilge_" ~ "Boat Bilge",
    sample_detail_description == "openwater_filter_glass_fiber" ~ "Open Water Glass Fiber Filter"
  ))
head(sdata)

unique(sdata$sample_detail_description)
...
write_rds(pps, "/home/lgschaer/old/Chesapeake/March_2023_Analysis/phyloseq_out/ps_rarefied_nochloro_justbacteria_06132023.rds")
...
random_tree = ape::rtree(ntaxa(pps), rooted=TRUE, tip.label=taxa_names(pps))
plot(random_tree)
...
plot_ordination(
  ...
  geom_point(aes(fill = sample_detail_description2, shape = boat_name), size = 6) +                         #sets fill color to sampletype
  scale_fill_manual(values = sample_colors) +
